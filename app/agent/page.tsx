...
export default function AgentDashboardPage() {
        ...
        setTimeout(() => {
            const mockAgents: AgentConfig[] = [
                ...
                },
                {
                    ...
                    type: 'YIELD',
                    description: 'Automatically harvests and compounds yield from various protocols',
                    parameters: {
                ...
                },
                {
                    ...
                    description: 'Monitors portfolio risk metrics and suggests adjustments',
                    parameters: {
            ...
            const mockPerformance: AgentPerformance = {
                ...
                strategies: [
                    ...
                    { id: '3', name: 'Stablecoin Safety', allocation: 10, performance: 1.5 }
                ],
                activities: [
                    {
                        id: '1',
    ...
    return (
        <div className="container mx-auto p-4 space-y-6">
            <h1 className="text-3xl font-normal mb-6 text-gradient-primary">AI Agent Dashboard</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <StatsCard
                    ...
                    value={formatDate(agents.find(a => a.type === 'OPTIMIZER')?.lastRunAt)}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-6">
                    <DashboardCard
                        title="Your Agents"
                        action={
                            <button className="px-4 py-2 bg-primary-500/30 rounded text-sm hover:bg-primary-500/50 transition-colors">
                                Add Agent
                            </button>
                        }
                        loading={isLoading}
                        ...
                        <div className="space-y-4">
                            ...
                            {agents.length === 0 && !isLoading && (
                                <div className="text-center py-8">
                                    <p className="text-white/70 mb-4">No agents have been created yet.</p>
                                    <button className="px-6 py-2 bg-primary-500/30 rounded hover:bg-primary-500/50 transition-colors">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {agentPerformance.strategies.map((strategy: Strategy) => (
                                <GlassPanel key={strategy.id} className="p-3" bordered>
                                    <div className="text-sm font-medium">{strategy.name}</div>
                                    <div className="flex justify-between mt-1">
                                        <span className="text-xs text-white/60">Allocation</span>
                                        <span className="text-xs">{strategy.allocation}%</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-xs text-white/60">Performance</span>
                                        <span className={`text-xs ${strategy.performance > 0 ? 'text-green-400' : 'text-red-400'}`}>
                    ...
                    >
                        <div className="space-y-3">
                            {agentPerformance.activities.slice(0, 5).map((activity: Activity) => (
                                <GlassPanel key={activity.id} className="p-3" bordered>
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center space-x-2">
                                            <div className={`w-2 h-2 rounded-full ${activity.type === 'REBALANCE' ? 'bg-primary-500' :
                                                    activity.type === 'YIELD_HARVEST' ? 'bg-green-500' :
                                                        activity.type === 'RISK_ALERT' ? 'bg-yellow-500' : 'bg-blue-500'
                                                }`} />
                                            <span className="font-medium">{
                                                activity.type === 'REBALANCE' ? 'Portfolio Rebalanced' :
                                                    activity.type === 'YIELD_HARVEST' ? 'Yield Harvested' :
                                                        activity.type === 'RISK_ALERT' ? 'Risk Alert' : activity.type
                                            }</span>
                                        </div>
                                    ...
                                    <p className="text-xs text-white/70 mt-1">{activity.description}</p>
                                    ...
                                    {activity.type === 'REBALANCE' && activity.changes && (
                                        <div className="text-xs text-white/60 mt-1 space-y-0.5">
                                            {activity.changes.map((change, i: number) => (
                                                <div key={i}>Rebalanced {change.asset}: {change.before}% → {change.after}%</div>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    {activity.type === 'RISK_ADJUSTMENT' && activity.riskLevel && (
                                        <p className="text-xs text-white/60 mt-1">
                                            New Risk Level: <span className={`font-semibold ${getRiskColor(activity.riskLevel)}`}>{activity.riskLevel}</span>
                                        </p>
                                    )}
                                    {activity.recommendation && (
                                        <p className="text-xs bg-white/5 p-2 rounded mt-2">Recommendation: {activity.recommendation}</p>
                                    )}
                                </GlassPanel>
                            ))}
...
function AgentCard({ agent }: { agent: AgentConfig }) {
    ...
    return (
        <GlassPanel className="p-4 transition-all duration-300" bordered>
            <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpanded(!expanded)}>
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <div className="flex items-center space-x-2">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs ${agent.isActive ? 'bg-green-900/30 text-green-400' : 'bg-red-900/30 text-red-400'
                                }`}>
                                <span className={`w-1.5 h-1.5 mr-1 rounded-full ${agent.isActive ? 'bg-green-400 animate-pulse' : 'bg-red-400'
                                    }`} />
                                {agent.isActive ? 'Active' : 'Inactive'}
                            </span>
                            <span className="text-xs text-white/60">{agent.type.replace('_', ' ')}</span>
                        </div>
                    </div>
                </div>
            ...
            {expanded && (
                <div className="mt-4 space-y-3">
                        ...
                        <div className="space-y-1 text-xs">
                            {Object.entries(agent.parameters).map(([key, value]) => (
                                <div key={key} className="flex justify-between">
                                    <span className="text-white/60">{key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}</span>
                                    <span>
                    ...
                    <div className="flex gap-2 pt-2">
                        <button className="flex-1 px-3 py-1.5 bg-primary-500/30 rounded text-sm hover:bg-primary-500/50 transition-colors">
                            Configure
                        </button>
                        <button className={`flex-1 px-3 py-1.5 rounded text-sm ${agent.isActive
