...
import { BookOpen, LineChart, Star, ShieldCheck, TrendingUp, TrendingDown } from 'lucide-react';
...
function StatsCardItem({ title, value, icon, change, gradient }: StatsCardItemProps) {
  const isMobile = useIsMobile();
  const isPositive = change && change > 0;
  const isNegative = change && change < 0;

  return (
    <ResponsiveCard variant="glass" size="sm" withHover className="relative overflow-hidden">
      ...
      <div className={cn("h-1 w-full absolute top-0 left-0 bg-gradient-to-r", gradient)} />
      
      <div className="pt-3 px-4">
        <div className="flex items-center mb-1.5">
          <div className={cn("p-1.5 rounded-md bg-gradient-to-br opacity-80", gradient)}>
            {icon}
          </div>
          <h3 className="text-xs sm:text-sm font-medium text-gray-400 ml-2">{title}</h3>
        </div>
